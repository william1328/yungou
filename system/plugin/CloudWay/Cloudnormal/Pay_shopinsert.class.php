...
function pay_insert_shop($shop='',$type=''){
    ...
    include_once dirname(__FILE__)."/"."/CloudTocode".".class.php";
	...
	if($u_info){		
		...
		if(is_dir(G_PLUGIN.'GoodsPecify/')):
			$package = _PluginGetOne('GoodsPecify');
			  if($package){		
				$path = G_PLUGIN . "GoodsPecify/". $package['Index'];
				include_once $path;
				$post_arr = Plugin_GoodsPecify_Hou($shop['id']);		
			  }
		endif;	
