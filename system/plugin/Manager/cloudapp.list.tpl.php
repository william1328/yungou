...
<link rel="stylesheet" href="<?php echo G_GLOBAL_STYLE; ?>/global/css/bootstrap.min.css" type="text/css">
<script src="<?php echo G_GLOBAL_STYLE; ?>/global/js/jquery-1.8.3.min.js"></script>
<script src="<?php echo G_GLOBAL_STYLE; ?>/global/js/bootstrap.min.js"></script>
<script src="<?php echo G_GLOBAL_STYLE; ?>/global/js/eModal.min.js"></script>
<style>
...
<script type="text/javascript">
	...
	var plugins = {};
		...
		plugins.view = function(data,i,keys){
			...
			if(keys=="yes"){
				...
				html += '<p><i>作者:</i> <span class="text">'+data.Author+'</span></p>';
				html += '<p><i>版本:</i> <span class="text">'+data.Version+'</span></p>';
				html += '<p><i>价格:</i> <span class="text C_f00">'+"￥"+data.Money+'</span></p>';
				html += '<p><i>介绍:</i><a title="点击查看安装注意事项" style="cursor:pointer;text-decoration:none;" onclick="desconclick('+data.PID+')"> <span class="text">'+data.Desc+'</span></a></p>';
				html += '<p>';
				html += '<i>操作:</i>';
				html += '<span class="text">';
				html += '<span class="btn B_ccc">已　装</span>';
				if(data.Money>0&&data.Free==0){					
				html += '<span class="btn B_f60" onclick=plugins_btn_pay("'+i+'")>购　买</span>';	
				}else{
				html += '<span class="btn B_f60" onclick=plugins_btn_freeget("'+data.PID+'")>免费获取</span>';						
				}
	...
	function plugins_btn_freeget(PID){
		var url="<?php echo G_WEB_PATH; ?>/?plugin=1&api=Manager&action=freeget";
    ...
    function desconclick(PID){  	
		var url="<?php echo G_WEB_PATH; ?>/?plugin=1&api=Manager&action=Ploneinfo&PID="+PID;
			var title ='介绍';
			var params = {
				buttons: [
				   { text: '关闭', close: true, style: 'danger' }             
				],
				size: eModal.size.lg,
				title: title,
				url:url 
