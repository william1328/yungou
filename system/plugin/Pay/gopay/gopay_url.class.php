			...
			if(!$dingdaninfo){
             ...
             if($dingdaninfo||$recorddingdan){
        		if($dingdaninfo){					
        		    ...
        		    $up_q1 = $this->db->Query("UPDATE `@#_orders` SET `opay` = '国富宝', `ostatus` = '2', `oremark` = '国富宝充值' where `oid` = '$dingdaninfo[oid]' and `ocode` = '$dingdaninfo[ocode]'");
        			$up_q2 = $this->db->Query("UPDATE `@#_user` SET `money` = `money` + $c_money where (`uid` = '$uid')");				
    		    	$up_q3 = $this->db->Query("INSERT INTO `@#_user_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '1', '账户', '国富宝充值', '$c_money', '$time')");                 
