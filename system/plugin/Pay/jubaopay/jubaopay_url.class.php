	...
	private function jubaopay_chuli($amount){
		...
		if(!$dingdaninfo){
		 ...
		 if($dingdaninfo||$recorddingdan){
			...
			if($dingdaninfo){
				$up_q1 = $this->db->Query("UPDATE `@#_orders` SET `opay` = '2', `ostatus` = '2',`oremark` = '聚宝支付充值'  where `oid` = '$dingdaninfo[oid]' and `ocode` = '$dingdaninfo[ocode]'");
				$up_q2 = $this->db->Query("UPDATE `@#_user` SET `money` = `money` + $c_money where (`uid` = '$uid')");				
				$up_q3 = $this->db->Query("INSERT INTO `@#_user_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '1', '账户', '聚宝支付充值', '$c_money', '$time')");                 
				if($up_q1&&$up_q2&&$up_q3){
				 ...
				 if($recorddingdan){
					...
					$up_q2 = $this->db->Query("UPDATE `@#_user` SET `money` = `money` + $c_money where (`uid` = '$uid')");				
					$up_q3 = $this->db->Query("INSERT INTO `@#_user_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '1', '账户', '聚宝支付充值', '$c_money', '$time')");             		  
					$recordupdate = $this->db->Query("UPDATE `@#_user_money_record` SET `status` ='2' WHERE `code` = '$out_trade_no'"); 					
